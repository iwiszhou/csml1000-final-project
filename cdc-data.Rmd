---
title: Clustering CDC Data - Nutrition, Physical Activity, & Obesity using Unsupervised
  Learning
output:
...
The data describe 9 different questions and each one has a corresponding precentage
...
Again, QuestionID is a value tranfromation of Question column. We keep Question and remove QuestionID.

```{r message=FALSE, warning=FALSE}
df$QuestionID <- NULL
```

There are total 9 levels, which means this dataset might be able to split by 9 different group. Each question represent one area.
...
So that, we can keep these two columns for now. And we would reley on Data_Value more than those High_Confidence_Limit and Low_Confidence_Limit
...
```{r message=FALSE, warning=FALSE}
df %>% ggplot(aes(x=YearStart, y=Data_Value)) + 
  geom_point() + 
...
This graph tells us a very important information. Some questions are missing data in some year.
Let's choose one Question and take a look the relationship for each categories.

```{r message=FALSE, warning=FALSE}
q1 <- df %>% filter(Question == "Percent of adults aged 18 years and older who have obesity")
...
q1 %>% ggplot(aes(x=StratificationCategory1, y=Data_Value, color = Stratification1 ) ) + 
  ...
  labs(title="Category vs Data_Value", x="Catetory", y="Data_Value")
```

This graph tells Race/Ethnicity category has wider range than others. Let's take look each category.

###### Category - Age (years)

```{r message=FALSE, warning=FALSE}
q1_age <- q1 %>% filter(StratificationCategory1 == "Age (years)");
q1_age %>% ggplot(aes(x=Stratification1, y=Data_Value ) ) + 
  geom_point() + 
...
It is clear obesity percentage increases when age increase. Peek is around 35-44 and 45-54. And it drops after.

###### Category - Education

```{r message=FALSE, warning=FALSE}
q1_education <- q1 %>% filter(StratificationCategory1 == "Education");
q1_education %>% ggplot(aes(x=Stratification1, y=Data_Value ) ) + 
  geom_point() + 
  facet_wrap(~ YearStart) +
  geom_smooth(method = lm, se = FALSE) + 
  theme(axis.text.x = element_text(angle=45)) +
...
This graph tell us lower educaiton level seems has higher percentage of obesity 

###### Category - Income

```{r message=FALSE, warning=FALSE}
q1_income <- q1 %>% filter(StratificationCategory1 == "Income");
q1_income %>% ggplot(aes(x=Stratification1, y=Data_Value ) ) + 
  geom_point() + 
  facet_wrap(~ YearStart) +
  geom_smooth(method = lm, se = FALSE) + 
  theme(axis.text.x = element_text(angle=45)) +
...
People has lower income has higher percentage of obesity 

###### Category - Race/Ethnicity

```{r message=FALSE, warning=FALSE}
q1_race <- q1 %>% filter(StratificationCategory1 == "Race/Ethnicity");
q1_race %>% ggplot(aes(x=Stratification1, y=Data_Value ) ) + 
  geom_point() + 
  facet_wrap(~ YearStart) +
  geom_smooth(method = lm, se = FALSE) + 
  theme(axis.text.x = element_text(angle=45)) +
...
This graph tell us Asian people has lowest percentage of obesity 

###### Category - Gender

```{r message=FALSE, warning=FALSE}
q1_gender <- q1 %>% filter(StratificationCategory1 == "Gender");
q1_gender %>% ggplot(aes(x=Stratification1, y=Data_Value ) ) + 
  geom_point() + 
  facet_wrap(~ YearStart) +
  geom_smooth(method = lm, se = FALSE) + 
  theme(axis.text.x = element_text(angle=45)) +
...
For this graph, we only can see female has wider range than male.

For "Percent of adults aged 18 years and older who have obesity" question, we can see middle aged people, who has lower education level and income less than $15,000, they will have higher chances to have a obesity health problem.