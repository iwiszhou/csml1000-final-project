...
q1_age %>% ggplot(aes(x=Stratification1, y=Data_Value ) ) + 
  ...
  geom_smooth(method = lm, se = FALSE) + 
  theme(axis.text.x = element_text(angle=45)) +
  labs(title="Category Age vs Data_Value", x="Stratification1", y="Data_Value")
```

It is clear Data_Value increases when age increase. Peek is around 35-44 and 45-54. And it drops after.
...
q1_education %>% ggplot(aes(x=Stratification1, y=Data_Value ) ) + 
  ...
  geom_smooth(method = lm, se = FALSE) + 
  theme(axis.text.x = element_text(angle=45)) +
  labs(title="Category Education vs Data_Value", x="Stratification1", y="Data_Value")
```

This graph tell us lower educaiton level seems has higher Data_Value.
