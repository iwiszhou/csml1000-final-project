...
```
  3.  **Building Models**
```{r age/data_value, echo = FALSE}
  ggplot( data = filter(clean_df, Age != "Unknown"), aes(x=Age, y= Data_Value)) + geom_boxplot()
```
  *concern about the obesity (Data_Value) is increasing with age and reach to highest at 55-64 years old. And obesity becomes less concern when getting 65 or older.*
```{r age/data_value_model, echo = FALSE}
   age_mod <- lm( Data_Value ~ Age, data = clean_df)
   age_grid <- clean_df %>% filter(Age != "Unknown") %>% data_grid( Age )
   age_grid <- age_grid %>% 
                add_predictions(age_mod)
   ggplot( data = filter(clean_df, Age != "Unknown" ), aes(x=Age)) + 
                geom_point( aes(y = Data_Value)) + 
                geom_point( age_grid, mapping = aes(x=Age, y=pred), color = "red", size = 2, fill = "red")
   

```
  

### **Unsupervised Learning - build a regression/random forse model**
  1.  **Import the source file** - Nutrition__Physical_Activity__and_Obesity_-_Behavioral_Risk_Factor_Surveillance_System.csv:
```{r step1, echo=FALSE}
library(tidyverse)
library(dplyr)
